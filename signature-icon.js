...
.identity{padding-right:116px!important}
.identity:after{content:none!important}
.identity .eyebrow,.identity h1,.identity p,.identity .tag{position:relative;z-index:2;max-width:calc(100% - 76px)}
.signature-icon{position:absolute;right:14px;top:14px;width:74px;height:74px;opacity:.88;filter:drop-shadow(0 0 16px #4fd28a25);pointer-events:none;z-index:1}
.signature-icon svg{width:100%;height:100%;display:block}
@media(max-width:767px){
  .identity{padding-right:88px!important}
  .identity .eyebrow,.identity h1,.identity p,.identity .tag{max-width:calc(100% - 54px)}
  .signature-icon{width:52px;height:52px;right:10px;top:10px;opacity:.72}
}
@media(prefers-reduced-motion:no-preference){.signature-icon{animation:sigFloat 4.5s ease-in-out infinite}@keyframes sigFloat{50%{transform:translateY(-3px);filter:drop-shadow(0 0 22px #4fd28a3d)}}}`;document.head.appendChild(style);
