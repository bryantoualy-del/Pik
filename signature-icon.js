...
.identity{padding-right:128px!important}
.identity:after{content:none!important}
.identity .eyebrow,.identity h1,.identity p,.identity .tag{position:relative;z-index:2;max-width:calc(100% - 86px)}
.signature-icon{position:absolute;right:16px;top:12px;width:82px;height:82px;opacity:.9;filter:drop-shadow(0 0 18px #4fd28a2d);pointer-events:none;z-index:1}
.signature-icon svg{width:100%;height:100%;display:block}
@media(max-width:767px){
  .identity{padding-right:102px!important}
  .identity .eyebrow,.identity h1,.identity p,.identity .tag{max-width:calc(100% - 68px)}
  .signature-icon{width:64px;height:64px;right:10px;top:9px;opacity:.82}
}
@media(min-width:768px) and (max-width:1366px){
  .identity{padding-right:144px!important}
  .identity .eyebrow,.identity h1,.identity p,.identity .tag{max-width:calc(100% - 98px)}
  .signature-icon{width:94px;height:94px;right:16px;top:12px;opacity:.92}
  .vitals{display:flex!important;flex-wrap:wrap!important;align-items:stretch!important;gap:7px!important}
  .vitals>.hpstat{flex:1 1 250px!important;min-width:230px!important}
  .vitals>.stat:not(.hpstat){flex:0 0 82px!important;width:82px!important;min-width:82px!important;max-width:82px!important;padding:7px 8px!important}
  .vitals>.stat:not(.hpstat) b{font-size:1.08rem!important}
  .vitals>.stat:not(.hpstat) small{font-size:.58rem!important;letter-spacing:.04em!important}
}
@media(prefers-reduced-motion:no-preference){.signature-icon{animation:sigFloat 4.5s ease-in-out infinite}@keyframes sigFloat{50%{transform:translateY(-3px);filter:drop-shadow(0 0 24px #4fd28a42)}}}`;document.head.appendChild(style);
